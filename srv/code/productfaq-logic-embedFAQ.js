/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "frostYuan_STUDENTG30Srv.ProductFAQ")
 * 
...
module.exports = async function(results, request) {
	...
	try {
		...
		productFAQ = await SELECT.one('FrostYuan_STUDENTG30.ProductFAQ').where({ ID: productFAQID }).forUpdate();
		if (!productFAQ) {
	...
	try {
		...
		await UPDATE('FrostYuan_STUDENTG30.ProductFAQ').set({ embedding: embedding }).where({ ID });
