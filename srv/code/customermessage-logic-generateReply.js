/**
 * 
 * @On(event = { "Action1" }, entity = "frostYuan_STUDENTG30Srv.CustomerMessage")
...
try {
    ...
    customerMessage = await SELECT.one.from('FrostYuan_STUDENTG30.CustomerMessage').where({ ID });
    if (!customerMessage) {
...
if (messageCategory === 'Technical') {
    ...
    try {
        ...
        relevantFAQs = await SELECT.from('###.ProductFAQ')
...
try {
    ...
    await UPDATE('FrostYuan_STUDENTG30.CustomerMessage').set({
